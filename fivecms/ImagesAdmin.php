...
class ImagesAdmin extends Fivecms
{	
	public function fetch()
	{
		$images_dir = 'design/'.$this->settings->theme.'/images/';
		$allowed_extentions = array('png', 'gif', 'jpg', 'jpeg', 'ico', 'webp', 'jp2', 'jxr', 'woff', 'woff2', 'ttf', 'otf', 'eot', 'svg');
		$images = array();
			...
			if($images = $this->request->files('upload_images'))
			{
				for($i=0; $i<count($images['name']); $i++)
				{
					...
					if(in_array(strtolower(pathinfo($name, PATHINFO_EXTENSION)), $allowed_extentions)){
						...
						$message_error = 'not_allowed_extention';
					}
				}
			}
