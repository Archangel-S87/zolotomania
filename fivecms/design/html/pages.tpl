...
{capture name=tabs}
	...
	{foreach from=$menus item=m}
		<li {if $m->id == $menu->id}class="active"{/if}><a style="font-size:14px;padding: 11px 10px 11px 10px;" href="{url module=PagesAdmin menu_id=$m->id}">{$m->name}</a></li>
	{/foreach}
	{/if}
{/capture}
...
<div id="header">
	<h1>{$menu->name}</h1>
	<a class="add" href="{url module=PageAdmin}">{$tr->add_page_or_url|escape}</a>
</div>
{if $pages}
<div id="main_list" class="pagespage">
 	<div style="margin-bottom: 15px;">
 		{if $menu->id == 18}
 			<strong>{$tr->dont_disable_pages|escape}</strong>
 		{else}
 			{$tr->page_as_url}
 		{/if}
 	</div>
		...
		<div id="list">		
			...
			<div class="{if !$page->visible}invisible{/if} row">
				...
				<div class="move cell"><div class="move_zone"></div></div>
		 		<div class="checkbox cell" {if $menu->id == 18}style="display:none;"{/if}>
					<input {if $menu->id == 18}disabled{/if} type="checkbox" name="check[]" value="{$page->id}" />				
				</div>
				<div class="name cell">
					<a href="{url module=PageAdmin id=$page->id return=$smarty.server.REQUEST_URI}">{$page->name|escape}{if $page->url == '404'} [{$tr->page|escape} 404]{/if}</a>
				</div>
				<div class="icons cell">
					<a class="preview" title="{$tr->open_on_page|escape}" href="../{$page->url}" target="_blank"></a>
					<a class="enable" title="{$tr->active|escape}" href="#"></a>
					{if $menu->id != 18}<a class="delete" title="{$tr->delete|escape}" href="#"></a>{/if}
				</div>
				<div class="clear"></div>
			</div>
			{/foreach}
		...
		<div id="action">
			...
			<span id="select">
				<select name="action">
					...
					{if $menu->id != 18}<option value="delete">{$tr->delete|escape}</option>{/if}
				</select>
			</span>
