...
{* Title *}
{$meta_title=$tr->payment_systems|escape scope=root}

{* Заголовок *}
<div id="header" style="margin-bottom:0;">
	<h1>{$tr->payment_systems|escape}</h1>
	<a class="add" href="{url module=PaymentMethodAdmin}">{$tr->add|escape}</a>
</div>	
...
<div id="main_list" style="max-width: 680px;">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<ul style="margin-bottom:15px;">
			<li>
				<label style="margin-right:10px;" class="property">{$tr->allow_payment|escape}</label>
				<select name="payment_control" class="fivecms_inp" style="width:auto;">
					<option value='0' {if $settings->payment_control == '0'}selected{/if}>{$tr->always|escape}</option>
					<option value='1' {if $settings->payment_control == '1'}selected{/if}>{$tr->if_status|escape}</option>
					<option value='2' {if $settings->payment_control == '2'}selected{/if}>{$tr->if_delivery|escape}</option>
				</select>
			</li>
		</ul>
