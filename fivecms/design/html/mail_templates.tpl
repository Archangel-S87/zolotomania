{capture name=tabs}
	...
	<li class="active"><a href="index.php?module=MailTemplatesAdmin">{$tr->mail_templates|escape}</a></li>
{/capture}

{if $template_file}
{$meta_title = "Шаблон $template_file (моб)" scope=root}
{/if}
...
<h1>{$tr->theme}: {$theme}, <span style="text-transform:lowercase;">{$tr->template}</span>: {$template_file}</h1>
...
<div class="block layer" style="padding-bottom:20px;">
	{$tr->mail_helper}
</div>
...
<div class="block layer">
	<div class="templates_names">
		{foreach item=t from=$templates}
			<a {if $template_file == $t}class="selected"{/if} href='index.php?module=MailTemplatesAdmin&file={$t}'>{$t}</a>
		{/foreach}
	</div>
</div>
