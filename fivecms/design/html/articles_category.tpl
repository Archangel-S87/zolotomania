{capture name=tabs}
	...
	<li class="active"><a href="index.php?module=ArticlesCategoriesAdmin">{$tr->articles_categories|escape}</a></li>
...
<div id="onecolumn" class="articlescategory">
	...
	<div class="message message_success">
		...
		<a class="link" target="_blank" href="../articles/{$category->url}">{$tr->open_on_page|escape}</a>
	...
	<input type=hidden name="session_id" value="{$smarty.session.id}">
		...
		<div id="product_categories">
				<select name="parent_id">
					...
					{foreach from=$articles_cats item=cat}
						...
						{else}
							...
							{if !empty($cat->subcategories)}
								{articles_category_select articles_cats=$cat->subcategories level=$level+1}
							{/if}
						{/if}
					{/foreach}
					{/function}
					{articles_category_select articles_cats=$articles_categories}
				</select>
		</div>
		...
		<div id="column_left">
			...
			<div class="block layer">
				...
				<ul>
					<li><label style="width: 90px;" class=property>{$tr->url|escape}</label><div class="page_url">/articles/</div><input style="width:414px;" name="url" class="page_url" type="text" value="{if !empty($category->url)}{$category->url|escape}{/if}" /></li>
			...
			<div class="block layer images">
				...
				<ul>
					<li>
						...
						<img src="../{$config->articles_categories_images_dir}{$category->image}" alt="" />
					</li>
				</ul>
				{/if}
			</div>
		...
		<!-- Description| Описание категории (The End)-->
		<input class="button_green button_save" type="submit" name="" value="{$tr->save|escape}" />
