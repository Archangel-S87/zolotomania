{* Вкладки *}
{capture name=tabs}
	<li class="active"><a href="{url module=ServicesCategoriesAdmin page=null keyword=null}">{$tr->services|escape}</a></li>
{/capture}

{* Title *}
{$meta_title=$tr->services|escape scope=root}

{* Заголовок *}
<div id="header">
	<h1>{$tr->services|escape}</h1>
	<a class="add" href="{url module=ServicesCategoryAdmin return=$smarty.server.REQUEST_URI}">{$tr->add|escape}</a>
</div>	
<!-- Заголовок (The End) -->
...
<div id="main_list" class="categories">
	...
	<form id="list_form" method="post">
		...
		{function name=services_categories_tree level=0}
		{if $services_categories}	
			<ul id="list" class="sortable">
				...
				<li class="{if !$category->visible}invisible{/if} row">		
					<div class="tree_row">
						...
						<div class="name cell">
							<a href="{url module=ServicesCategoryAdmin id=$category->id return=$smarty.server.REQUEST_URI}">{$category->menu|escape}</a> 	 			
						</div>
						<div class="icons cell">
							<a class="preview" title="{$tr->open_on_page|escape}" href="../services/{$category->url}" target="_blank"></a>				
					...
					{if !empty($category->subcategories)}
						{services_categories_tree services_categories=$category->subcategories level=$level+1}
					{/if}
				</li>
				{/foreach}
...
$(function() {
	...
	$("a.enable").click(function() {
		...
		$.ajax({
			...
			data: {'object': 'services_category', 'id': id, 'values': {'visible': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},
	...
	$("a.delete").click(function() {
		...
		$(this).closest(".row").find('input[type="checkbox"][name*="check"]:first').attr('checked', true);
