...
class PageAdmin extends Fivecms
{	
	...
	{	
		...
		if($this->request->method('POST'))
		{
			...
			$skip_check = false;
			$exept_base = substr($page->url, 0, 8);
			if(stripos($exept_base, 'blog') !== false || stripos($exept_base, 'article') !== false || stripos($exept_base, 'services') !== false || stripos($exept_base, 'survey') !== false || stripos($exept_base, 'page') !== false) {$skip_check = true;}
			
			if(($p = $this->pages->get_page($page->url)) && $p->id!=$page->id && !$skip_check)
			{			
