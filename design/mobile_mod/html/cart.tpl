			...
			<ul class="purchaseslist">
				{foreach from=$cart->purchases item=purchase}
						{if !empty($purchase->product->name)}
							<li data-purchase-id="{$purchase->variant->id}" class="purchase {if $purchase->variant->stock == 0}out_of_stock{/if}">
									{* Изображение товара *}
									...
									<div class="product_info separator">
										{* Название товара *}
										<h3 class="purchasestitle"><a href="products/{$purchase->product->url}">{$purchase->product->name|escape}</a>{$purchase->variant->name|escape}</h3>

										{* Цена за единицу *}
										<div class="price">
											<span class="purprice">{($purchase->variant->price)|convert}</span> 
											<span class="purcurr">{$currency->sign}</span> 
											<span class="purx" style="display: none;">&nbsp;x&nbsp;
											</span>
										</div>
										{* Количество *}
