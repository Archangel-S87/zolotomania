...
{else}
	{if !empty($products)}
		...
		{if !empty($category->name) && empty($category->subcategories)}
				...
				<li class="product {if !empty($settings->show_cart_wishcomp)}visible_button{/if}">
					<div class="image qwbox" onclick="window.location='products/{$product->url}'">
						{if !empty($product->image)}
							<img loading="lazy" alt="{$product->name|escape}" title="{$product->name|escape}" class="lazy" src="{$product->image->filename|resize:150:150}" />
						{else}
							<img loading="lazy" src="/design/mobile_mod/images/no-photo.png" alt="{$product->name|escape}"
