...
<style>
    ...
    #addresses_list .address-header,
    #shop_list .shop-header {
        cursor: pointer;
    }
    #addresses_list .checkbox,
    #shop_list .checkbox {
        display: none;
    }
    #addresses_list label:hover .address-header,
    #shop_list label:hover .shop-header {
        color: #ED1C24;
    }
</style>
...
<div class="flex button-block">
    <div class="row" style="justify-content: center">
        <button id="bring_me" class="button{if $purchase_method =='bring_me'} active{/if}" style="margin-right: 20px;">Доставка
        </button>
        <button id="i_pickup" class="button{if $purchase_method =='i_pickup'} active{/if}">Заберу в
            магазине
        </button>
    </div>
...
<div class="form cart_form data-block">
    ...
    <div class="tab-data user-data-info user-data{if $purchase_method =='bring_me'} active{/if}">
    ...
    <div class="tab-data shop-data-info shop-data{if $purchase_method =='i_pickup'} active{/if}">
        <div class="row" style="display: block; margin-top: 20px; text-align: center;">
    ...
    <div class="all-data all-data-actions"{if !$purchase_method} style="display: none;" {/if}>
        {if $new_user}
            <div class="row">
                ...
                <div class="row" style="margin-top: 20px; justify-content: space-around">
                    <div class="row" style="justify-content: center">
                        <label for="have_manning" class="totalbonuses"
                               style="border: 2px solid #bcd4e4; margin-bottom: 10px; padding-bottom: 0; line-height: 24px; padding-left: 10px;">Мои маннинги</label>
                        <input id="have_manning"
                               type="text"
                               style="width: 80px;"
                               value="{$user->balance|convert}"
                               disabled="disabled">
                    </div>
                    <div class="row" style="justify-content: center">
                        <label for="input_bonus" style="width: 182px; border: 2px solid #bcd4e4; margin-bottom: 10px; padding-bottom: 0; padding-left: 10px; line-height: 24px;">Потратить Маннинги</label>
    ...
    <div class="tab-data all-data-actions user-data{if $purchase_method =='bring_me'} active{/if}">
    ...
    <div class="tab-data all-data-actions shop-data{if $purchase_method =='i_pickup'} active{/if}">
        <div class="row" style="justify-content: space-around;">
            <input {if $settings->counters || $settings->analytics}onclick="{if $settings->counters}ym({$settings->counters},'reachGoal','cart'); {/if}{if $settings->analytics}ga ('send', 'event', 'cart', 'order_button');{/if} return true;"{/if}
                   type="submit" name="check_buy" class="button hideablebutton checkout"
                   value="Оплата онлаин"/>
