				...
				<ul id="deliveries">
					{foreach $payment_methods as $payment_method}
						<li>
							...
							<span class="delivery-header"><label
										for=payment_{$payment_method->id}>	{$payment_method->name}, к оплате {$order->total_price|convert:$payment_method->currency_id}&nbsp;{if isset($all_currencies[$payment_method->currency_id]->sign)}{$all_currencies[$payment_method->currency_id]->sign}{/if}</label></span>{if $payment_method->description}
							<a class="hideBtn" href="javascript://"
							   onclick="hideShow(this);return false;">подробнее</a>{/if}
