...
foreach($rss as $r) {
	print '<item turbo="true">';
        ...
        print '<turbo:source>'.$fivecms->config->root_url.'/blog/'.$r->url.'</turbo:source>';
        print '<pubDate>'.$r->last_modify.'</pubDate>'; // D, d M Y H:i:s +0000
		print '<turbo:content><![CDATA[';
		
		$images = array();
   		$images = $fivecms->blog->get_images(array('post_id'=>$r->id));
   		
   		/*// Выводим первое изображение
   		if(!empty($images)){
   			print '<figure><img src="'.$fivecms->design->resize_modifier($images[0]->filename, 400, 400, false, $fivecms->config->resized_blog_images_dir).'"/></figure>';
   		}*/
		...
		if(!empty($content)){
   		...
   		if(!empty($images)){		  
			...
			foreach($images as $i){
				print '<img src="'.$fivecms->design->resize_modifier($i->filename, 400, 400, false, $fivecms->config->resized_blog_images_dir).'"/>';
			}
			print '</div>';
		}
		
		if(!empty($r->text)){
			print '<button formaction="'.$fivecms->config->root_url.'/blog/'.$r->url.'" data-background-color="#0893b9" data-color="#ffffff" data-turbo="false" data-primary="true" >Полная версия ...</button>';
   		}
