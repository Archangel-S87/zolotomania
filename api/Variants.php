...
class Variants extends Fivecms
{
    ...
    public function get_variants($filter = array(), $admin = 0)
    {
        ...
        if (!empty($filter['external_id']))
            $variant_id_filter = $this->db->placehold('AND v.external_id in(?@)', (array)$filter['external_id']);
        ...
        $query = $this->db->placehold("SELECT v.id, v.external_id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.name1, v.name2, v.unit, v.currency_id, v.attachment, v.position, v.discount, v.discount_date, v.shop_id, v.reservation 
					FROM __variants AS v
					WHERE 
					1
					$product_id_filter          
    ...
    public function get_variant($id, $admin = 0)
    {
        ...
        $query = $this->db->placehold("SELECT v.id, v.external_id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.name1, v.name2, v.unit, v.currency_id, v.attachment, v.discount, v.discount_date, v.reservation 
    ...
    /**
     * Получает внутрений id варианта по переданому внешнему id
     * @param $external_id string
     * @return bool|int
     */
    public function get_variant_id_by_external_id($external_id)
    {
