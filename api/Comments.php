...
class Comments extends Fivecms
{
	...
	{	
		...
		if(!empty($filter['type'])) {
			...
			elseif($filter['type'] == 'response')
            {
                $products_fields = ', b.url, b.name product';
                $products_join = 'INNER JOIN __pages b ON c.object_id=b.id';
            }
		}
		...
		$query = $this->db->placehold("SELECT c.id, c.object_id, c.ip, c.name, c.email, c.text, c.otvet, c.type, c.date, c.text, c.approved
										$products_fields FROM __comments c $products_join WHERE 1 $object_id_filter $type_filter $keyword_filter $approved_filter ORDER BY id $sort $sql_limit");
